inherited frmRelPedidoPeriodo: TfrmRelPedidoPeriodo
  Left = 244
  Top = 161
  Caption = 'Pedidos por Per'#237'odo'
  ClientHeight = 324
  ClientWidth = 578
  ExplicitWidth = 594
  ExplicitHeight = 362
  PixelsPerInch = 96
  TextHeight = 13
  object dbeCliente: TPLDBEdit [0]
    Left = 104
    Top = 80
    Width = 265
    Height = 21
    Hint = 'Pesquisar < F2 >'
    Color = clBtnFace
    Ctl3D = True
    DataField = 'CLIENTE'
    DataSource = dsSelecao
    ReadOnly = True
    TabOrder = 3
    EditLabel.Left = 66
    EditLabel.Top = 84
    EditLabel.Width = 35
    EditLabel.Height = 13
    EditLabel.Alignment = taRightJustify
    EditLabel.Caption = 'Cliente:'
    EditLabel.FocusControl = dbeCliente
    Button.Left = 370
    Button.Top = 79
    Button.Width = 21
    Button.Height = 21
    Button.Cursor = crArrow
    Button.Hint = 'Pesquisa <F2>'
    Button.Caption = '...'
    Button.Glyph.Data = {
      
      7878787878787878787878787878787878787878FFFFFFFFFFFF}
    Button.NumGlyphs = 2
    Button.ParentShowHint = False
    Button.ShowHint = True
    Button.Visible = False
    Button.OnClick = dbeClienteClickButton
    Button.AutoSize = True
    OnClickButton = dbeClienteClickButton
    ReadOnlyChangesColor = False
  end
  object dtpDataFim: TPLDBEditDateTimePicker [1]
    Left = 104
    Top = 56
    Width = 129
    Height = 21
    Date = 40342.766646319450000000
    Time = 40342.766646319450000000
    TabOrder = 2
    DataField = 'DATAFIM'
    DataSource = dsSelecao
    ReadOnly = False
    DefaultEditMask = '!99/99/9999;1;_'
    AutoApplyEditMask = True
    EditLabel.Left = 50
    EditLabel.Top = 60
    EditLabel.Width = 51
    EditLabel.Height = 13
    EditLabel.Alignment = taRightJustify
    EditLabel.Caption = 'Data Final:'
    EditLabel.FocusControl = dtpDataFim
    Required = False
  end
  object dtpDataIni: TPLDBEditDateTimePicker [2]
    Left = 104
    Top = 32
    Width = 129
    Height = 21
    Date = 40342.766561898150000000
    Time = 40342.766561898150000000
    TabOrder = 1
    DataField = 'DATAINI'
    DataSource = dsSelecao
    ReadOnly = False
    DefaultEditMask = '!99/99/9999;1;_'
    AutoApplyEditMask = True
    EditLabel.Left = 45
    EditLabel.Top = 36
    EditLabel.Width = 56
    EditLabel.Height = 13
    EditLabel.Alignment = taRightJustify
    EditLabel.Caption = 'Data Inicial:'
    EditLabel.FocusControl = dtpDataIni
    Required = False
  end
  inherited pnlBotoes: TPanel
    Top = 270
    Width = 578
    ExplicitTop = 270
    ExplicitWidth = 578
  end
  inherited dsPadrao: TDataSource
    DataSet = cdsPadrao
    Left = 216
    Top = 136
  end
  object sqldPadrao: TPLSQLDataSet
    CommandText = 
      'select'#13#10'  ip.CODPRODUTO,'#13#10'  prod.NOME as PRODUTO,'#13#10'  sum(ip.QTDE' +
      ') as TOTALQTDE,'#13#10'  un.DESCRICAO as UNIDADE'#13#10'from ITEMPEDIDO ip'#13#10 +
      'join PEDIDO p on (p.CODPEDIDO = ip.CODPEDIDO)'#13#10'join PRODUTO prod' +
      ' on (prod.CODPRODUTO = ip.CODPRODUTO)'#13#10'left join UNIDADES un on ' +
      '(un.CODUNIDADES = prod.CODUNIDADE)'#13#10'where'#13#10'  p.DATAPEDIDO betwee' +
      'n :DATAINI and :DATAFIM'#13#10'and'#13#10'  ((p.CODCLIENTE = :CODCLIENTE) or' +
      ' (:CODCLIENTE = -1))'#13#10'group by'#13#10'  ip.CODPRODUTO,'#13#10'  prod.NOME,'#13#10 +
      '  un.DESCRICAO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDate
        Name = 'DATAINI'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'DATAFIM'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'CODCLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'CODCLIENTE'
        ParamType = ptInput
      end>
    Left = 120
    Top = 136
    object sqldPadraoCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
    end
    object sqldPadraoPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Size = 80
    end
    object sqldPadraoTOTALQTDE: TFMTBCDField
      FieldName = 'TOTALQTDE'
      Precision = 15
      Size = 2
    end
    object sqldPadraoUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Size = 80
    end
  end
  object dspPadrao: TPLDataSetProvider
    DataSet = sqldPadrao
    Left = 152
    Top = 136
  end
  object cdsPadrao: TPLClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'DATAINI'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'DATAFIM'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'CODCLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'CODCLIENTE'
        ParamType = ptInput
      end>
    ProviderName = 'dspPadrao'
    Left = 184
    Top = 136
    object cdsPadraoCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
    end
    object cdsPadraoPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Size = 80
    end
    object cdsPadraoTOTALQTDE: TFMTBCDField
      FieldName = 'TOTALQTDE'
      Precision = 15
      Size = 2
    end
    object cdsPadraoUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Size = 80
    end
  end
  object sqldSelecao: TPLSQLDataSet
    CommandText = 
      'select'#13#10'  cast(null as integer) CODCLIENTE,'#13#10'  cast(null as VARC' +
      'HAR(80)) CLIENTE,'#13#10'  cast(null as DATE) DATAINI,'#13#10'  cast(null as' +
      ' DATE) DATAFIM'#13#10'from RDB$DATABASE'
    MaxBlobSize = -1
    Params = <>
    Left = 320
    Top = 136
    object sqldSelecaoCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
    end
    object sqldSelecaoCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Size = 80
    end
    object sqldSelecaoDATAINI: TDateField
      FieldName = 'DATAINI'
    end
    object sqldSelecaoDATAFIM: TDateField
      FieldName = 'DATAFIM'
    end
  end
  object dspSelecao: TPLDataSetProvider
    DataSet = sqldSelecao
    Left = 352
    Top = 136
  end
  object cdsSelecao: TPLClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspSelecao'
    Left = 384
    Top = 136
    object cdsSelecaoCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      OnValidate = cdsSelecaoCODCLIENTEValidate
    end
    object cdsSelecaoCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Size = 80
    end
    object cdsSelecaoDATAINI: TDateField
      FieldName = 'DATAINI'
    end
    object cdsSelecaoDATAFIM: TDateField
      FieldName = 'DATAFIM'
    end
  end
  object dsSelecao: TDataSource
    DataSet = cdsSelecao
    Left = 416
    Top = 136
  end
end
